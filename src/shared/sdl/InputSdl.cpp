...
uint32_t Input::GetFirstTouchId( ) const {
    assert( TouchIdCount > 0 && "Invalid." );
    return TouchIds[ 0 ];
}

uint32_t Input::GetLastTouchId( ) const {
    assert( TouchIdCount > 0 && "Out of range." );
    return TouchIds[ TouchIdCount - 1 ];
}

bool Input::IsTouchTracked( uint32_t TouchId ) const {
...
bool Input::IsTrackingTouchesOrMousePressed( ) const {
    return bIsTrackingTouchesOrMousePressed;
}

bool Input::IsPressed( DigitalInputUInt InDigitalInput ) const {
    return Buttons[ 0 ][ InDigitalInput ];
}

bool Input::IsFirstPressed( DigitalInputUInt InDigitalInput ) const {
    return Buttons[ 0 ][ InDigitalInput ] && !Buttons[ 1 ][ InDigitalInput ];
}

bool Input::IsReleased( DigitalInputUInt InDigitalInput ) const {
    return !Buttons[ 0 ][ InDigitalInput ];
}

bool Input::IsFirstReleased( DigitalInputUInt InDigitalInput ) const {
    return !Buttons[ 0 ][ InDigitalInput ] && Buttons[ 1 ][ InDigitalInput ];
}

float Input::GetDurationPressed( DigitalInputUInt InDigitalInput ) const {
    return HoldDuration[ InDigitalInput ];
}

float Input::GetAnalogInput( AnalogInputUInt InAnalogInput ) const {
    return Analogs[ InAnalogInput ];
}

float Input::GetTimeCorrectedAnalogInput( AnalogInputUInt InAnalogInput ) const {
            ...
            for ( int i = 0; i < peepedEventCount; ++i ) {
                ...
                switch ( PeepedEvent.type ) {
                    case SDL_FINGERDOWN: {
                        ...
                        InOutState.TouchIdCount = (uint32_t) std::distance( InOutState.TouchIds, TouchIt );
                    } break;
                    ...
                    case SDL_FINGERUP: {
                        ...
                        if ( TouchIt != TouchItEnd ) {
                            uint32_t TouchIdx = (uint32_t) std::distance( InOutState.TouchIds, TouchIt );
                            InOutState.TouchIds[ TouchIdx ] = Input::sInvalidTouchValue;

                            std::sort( InOutState.TouchIds, TouchItEnd );
                            TouchIt = std::unique( InOutState.TouchIds, TouchItEnd );
                            InOutState.TouchIdCount = (uint32_t) std::distance( InOutState.TouchIds, TouchIt ) - ptrdiff_t( TouchItEnd == TouchIt );
                        }
                    } break;
